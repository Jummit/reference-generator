...
func _on_ToolMenu_generate_class_reference_pressed(_ud) -> void:
	var class_script := get_editor_interface().get_script_editor().\
			get_current_script()
	OS.clipboard = "const %s = preload(\"%s\")" % [snake_to_pascal_case(
			class_script.resource_path.get_file().get_basename()),
			class_script.resource_path]
...
func _on_ToolMenu_generate_node_reference_pressed(_ud) -> void:
	var selected_nodes := get_editor_interface().get_selection().\
			get_selected_nodes()
	var current_script := get_editor_interface().get_script_editor().\
			get_current_script()
	var relative_to_node := search_node_that_uses(
			get_editor_interface().get_edited_scene_root(), current_script)
	
	var reference_code := ""
	for selected_node in selected_nodes:
		reference_code += get_node_reference(selected_node,
				relative_to_node.get_path_to(selected_node)) + "\n"
